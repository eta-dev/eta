...
describe('Compile to String test', () => {
  it('parses a simple template', () => {
    var str = CompileToString('hi {{ hey }}', Env.default)
  ...
  it('works with filters', () => {
    var str = CompileToString('hi {{ hey | stuff | stuff2 ("param1") }}', Env.default)
    expect(str).toEqual(
  ...
  it('works with helpers', () => {
    var str = CompileToString('{{~each(x) => hi }} Hey {{#else }} oops {{/ each}}', Env.default)
    expect(str).toEqual(
  ...
  test('throws with bad filter syntax', () => {
    expect(() => {
      CompileToString('{{~hi () hey | hi /}}', Env.default)
    }).toThrow()
  })

  it('works with self-closing helpers', () => {
    var str = CompileToString('{{~log ("hey") | hi /}}', Env.default)
    expect(str).toEqual("var tR='';tR+=l('F','hi')(l('H','log')(\"hey\"));return tR")
  })

  it('works with helpers with results', () => {
    var str = CompileToString('{{~log ("hey") => res, res2}}{{/log}}', Env.default)
    expect(str).toEqual(
  ...
  test("throws when helpers start and end don't match", () => {
    expect(() => {
      CompileToString('{{~each(x) => hi }} Hey {{#else }} oops {{/ if}}', Env.default)
    }).toThrow()
  })
